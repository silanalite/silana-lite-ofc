...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.isGroup && global.db.data.chats[m.chat].adminonly && !isAdmin) {
                ...
                m.isCommand = true
                let xp = 'exp' in plugin ? parseInt(plugin.exp) : 23 // XP Earning per command
                if (xp > 200)
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    let templ = global.db.data.settings[this.user.jid]  
...
if (templ.gcImg) {
    ...
    this.reply(id, text, null,{contextInfo: {
    mentionedJid: [user]
    }})
